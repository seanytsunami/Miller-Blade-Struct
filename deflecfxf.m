...
%{
  ...
  beta (twist), v (pitch), py, pz (y and z loads)
  
  Returns: y and z deflections and moments, y and z blade deflection angles
  theta
...
% initializing arrays for vars w/ root BCs
M1=1:nj;
M2=1:nj;
kappa1=1:nj;
kappa2=1:nj;
...
for j=1:1:nj-1
    ...
    uy(j+1) = uy(j) + thetaz(j)*(x(j+1) - x(j)) + (1/6*kappaz(j+1) + 1/3*kappaz(j))*(x(j+1) - x(j)).^2;
    uz(j+1) = uz(j) - thetay(j)*(x(j+1) - x(j)) + (1/6*kappay(j+1) + 1/3*kappay(j))*(x(j+1) - x(j)).^2;
end
